...
#include <QQmlContext>
#include <QVideoFrame>
// #include "private/qvideoframe_p.h"   // In original source but removed in Qt5.15.0

QImage QVideoFrameToQImage( const QVideoFrame& videoFrame )
{
    if ( videoFrame.handleType() == QAbstractVideoBuffer::NoHandle )
    {
        // QImage image = qt_imageFromVideoFrame( videoFrame );  // In original source but removed in Qt5.15.0
